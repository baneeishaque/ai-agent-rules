...
When user requests to download files from rclone remotes, follow this context-aware workflow that balances efficiency,
reliability, and user control. The protocol differentiates between initial downloads (where destination doesn't
exist) and resume scenarios (where partial downloads exist).
...
1. **Ordering Trade-off**: `--checkers 1` required for strict size-based ordering

1. **Reliability Features**: Automatic retries and checksum verification

1. **Progress Visibility**: Human-readable progress with ETA
...
- **Session Log**:
    ...
    [2026-01-19-rclone-download-optimization.md](./conversations/2026-01-19-rclone-download-optimization.md)

- **Date Created**: 2026-01-19
- **Context**: User requested efficient workflow for downloading large datasets from rclone remotes with

    progress tracking and size-based ordering
